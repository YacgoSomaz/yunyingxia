...
describe('desktop security configuration', () => {
  it('requires a signed account login by default when no local config file exists', () => {
    ...
    expect(config.updatePublicKey).not.toBe('')
  ...
  it('uses an isolated Electron renderer with a narrow updater bridge', () => {
    ...
    const yunyingxiaBuild = readFileSync(join(root, 'build_yunyingxia_release.bat'), 'utf8')
    ...
    expect(releaseBuild).toContain("if (-not $UpdatePublicKey.Trim()) { throw '商业版构建必须提供 -UpdatePublicKey' }")
    expect(yunyingxiaBuild).toContain('set "UPDATE_PUBLIC_KEY=lYg7Ws_9MxeQYmSVP6SNJ8ZgRh1isI8mv_SwIrP7eZ4"')
    expect(yunyingxiaBuild).toContain('-UpdatePublicKey "%UPDATE_PUBLIC_KEY%"')
