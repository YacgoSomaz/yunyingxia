...
describe('license crypto', () => {
  it('verifies the Ed25519 envelope used by license.runmo.art', () => {
    ...
    const payload = Buffer.from(JSON.stringify({ product_code: 'wanshan_media', device_hash: 'device-1', expires_at: 2_000_000_000, grace_until: 2_000_100_000 }))
    const document = {
      alg: 'Ed25519',
      payload: b64url(payload),
      signature: b64url(crypto.sign(null, payload, privateKey)),
    }
    expect(verifyLicenseDocument(document, b64url(rawPublicKey), 'device-1', 'wanshan_media').product_code).toBe('wanshan_media')
  })
  ...
  it('rejects a license for another product or device', () => {
    ...
    expect(() => verifyLicenseDocument(document, b64url(rawPublicKey), 'device-1', 'wanshan_media')).toThrow('授权产品不匹配')
  })
})
