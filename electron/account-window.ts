...
function active(state){const product=operationProduct(state);if(product){const exp=Date.parse(product.expires_at||'');return product.status==='active'&&Number.isFinite(exp)&&exp>Date.now()&&Array.isArray(product.entitlements)&&product.entitlements.includes('operation_course')}if(state&&Array.isArray(state.products)&&state.products.length>0)return false;const user=state&&state.user;if(!user||user.is_member===false)return false;const now=Date.parse(user.server_time||'')||Date.now();const exp=Date.parse(user.membership_expires_at||'');return Number.isFinite(exp)&&exp>now&&Number(user.energy_balance||0)>0}
